...
void YZMotorNode::statusTimerCallback() {
    ...
    if (!cia402_driver_) {
        RCLCPP_ERROR(this->get_logger(), "Driver not initialized");
        return;
    }
    ...
    if (counter % 5 == 0) {
        ...
        // 检查状态字是否有效（非零）
        if (status == 0) {
            RCLCPP_WARN(this->get_logger(), "Received zero status word, possible communication issue");

            // 尝试重新初始化CAN通信
            static int retry_count = 0;
            if (++retry_count >= 5) {  // 连续5次失败后尝试重新初始化
                RCLCPP_ERROR(this->get_logger(), "Multiple status read failures, attempting to reinitialize CAN");
                if (canopen_driver_) {
                    canopen_driver_->init();
                }
                retry_count = 0;
            }
        ...
        // 打印当前状态（调试用）
        RCLCPP_DEBUG(this->get_logger(), "Current status: 0x%04X", status);
    }
}
