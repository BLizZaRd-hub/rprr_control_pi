...
bool CiA402Driver::updateStatusWord() {
    ...
    // 使用超时和重试机制读取状态字
    if (!canopen_->readSDO<uint16_t>(0x6041, 0, new_status_word, 100, 3)) {
        std::cerr << "Failed to read status word" << std::endl;
        return false;
    }

    // 如果状态字发生变化，打印调试信息
    if (new_status_word != status_word_) {
        std::cout << "Status word changed: 0x" << std::hex << status_word_
                  << " -> 0x" << new_status_word << std::dec << std::endl;

        // 打印状态字的详细信息
        std::cout << "  State: " << static_cast<int>(getStateFromStatusWord(new_status_word)) << std::endl;
        std::cout << "  Ready to Switch On: " << ((new_status_word & 0x0001) ? "Yes" : "No") << std::endl;
        std::cout << "  Switched On: " << ((new_status_word & 0x0002) ? "Yes" : "No") << std::endl;
        std::cout << "  Operation Enabled: " << ((new_status_word & 0x0004) ? "Yes" : "No") << std::endl;
        std::cout << "  Fault: " << ((new_status_word & 0x0008) ? "Yes" : "No") << std::endl;
        std::cout << "  Voltage Enabled: " << ((new_status_word & 0x0010) ? "Yes" : "No") << std::endl;
        std::cout << "  Quick Stop: " << ((new_status_word & 0x0020) ? "No" : "Yes") << std::endl;
        std::cout << "  Switch On Disabled: " << ((new_status_word & 0x0040) ? "Yes" : "No") << std::endl;
        std::cout << "  Warning: " << ((new_status_word & 0x0080) ? "Yes" : "No") << std::endl;
        std::cout << "  Target Reached: " << ((new_status_word & 0x0400) ? "Yes" : "No") << std::endl;
    }
